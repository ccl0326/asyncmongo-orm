...
from asyncmongoorm.field import StringField, ObjectIdField, IntegerField
...
class CollectionTestForFindAndModify(Collection):
	__collection__ = "collection_test_for_find_and_modify"
...
class ManagerTestCase(testing.AsyncTestCase):
	...
	def test_find_and_modify(self):
		collection_test = CollectionTestForFindAndModify()
		collection_test._id = ObjectId()
		collection_test.integer = 0
		collection_test.save(callback=self.stop)
		self.wait()
		CollectionTest.objects.find_and_modify(_query = {'_id': collection_test._id}, _update = {'$inc':{'integer':1}}, _upsert = True,_new = True, callback = self.stop)
		result = self.wait()
		self.assertEquals(result.integer, 1)
